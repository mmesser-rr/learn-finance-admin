...
// // Components
import { fetchEventThunk } from '../store/eventSlice';
...
function EventPage() {
  ...
  useDeepCompareEffect(() => {
    async function load() {
      ...
      await dispatch(fetchEventThunk(routeParams)).then(async (action) => {
        opp = action.payload.getEvent;
        console.log('opp', opp)
        if (opp) {
          setEvent(opp);
    ...
    <Root
      ...
      content={
        <div className="p-16 sm:p-24">
          <p className="mx-4">sponsor: {event?.sponsor}</p>
          <p className="mx-4">description: {event?.description}</p>
          <p className="mx-4">location: {event?.location}</p>
          <p className="mx-4">dateTime: {new Date(event?.dateTime).toString()}</p>
          <p className="mx-4">category: {event?.category}</p>
          <p className="mx-4">reward: {event?.reward}</p>
        </div>
      }
    />
  );
