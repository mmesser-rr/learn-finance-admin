...
import { removeReward, saveRewardThunk, createRewardThunk } from "../store/rewardSlice";
...
function RewardHeader(props) {
  ...
  useEffect(() => {
    async function getImage() {
      let imgSrc = logoUri;
      if (logoUri?.startsWith("rewards")) {
        console.log("rewardHeader => logoUri => ", logoUri);

        imgSrc = await Storage.get(logoUri, { download: false });
        console.log("rewardHeader => logoUriFetched => ", imgSrc);
      }
      setImageUrl(imgSrc);
    }
    getImage();
  ...
  async function handleSaveReward() {
    ...
    await dispatch(formValues?.id ? saveRewardThunk(formValues) : createRewardThunk(formValues)).then((action) => {
      console.log("rewardHeader => save => ", action);
      const item = formValues?.id ? action.payload : action.payload;
      navigate(
        `/pages/Reward/${item.id}/${item.title?.replaceAll(" ", "_")}`
      );
    });
  }
